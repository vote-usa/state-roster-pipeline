...
--
-- A CLI call collects a whole state and year in one fetch, because that is how the
...
-- SourceElectionId is the source system's own id for the election a row was collected
-- for. A date plus a type cannot always identify one: TX ran two special elections on
-- 2026-11-03, both typed Special. The collectors know the election when they build each
-- row, so the id is carried through and the matcher prefers it.
CREATE TABLE `RunCandidates` (
  ...
  `SourceElectionId` VARCHAR(50) NULL,
...
-- RunCandidates and RunMeasures but are kept verbatim because they are what that county's
-- voters actually see, including local races the statewide feed does not attribute.
...
-- Rows the collector produced whose election could not be identified.
-- They are recorded here with the reason and counted on the pass.
